...
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting biomni-agent in development mode..."
	@uv run watchfiles "python -m biomni_agent" biomni_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t biomni_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t biomni_agent:latest .
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi biomni_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag biomni_agent:latest para5/biomni_agent:latest
	@docker push para5/biomni_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep biomni_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep biomni_agent || echo "No images found"
	@echo "\nAgent Health:"
